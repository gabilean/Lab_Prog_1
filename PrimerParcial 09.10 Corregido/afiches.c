...
/**
* \brief Imprime el array de afiches a cobrar de forma encolumnada
* \param list Afiche*
* \param int len
* \return int return -1 si el array de afiches es NULL - 0 muestra correcta
*/
...
int afiches_printClientesACobrar(Cliente* listCliente, Afiche* listAfiche, int len)
{
    ...
    if(listCliente != NULL && len >= 0)
    {
        ...
        for(i=0; i<len; i++)
        {
            ...
            if(aCobrar > 0 && listAfiche[i].isEmpty == COBRAR)
            {
...
/**
* \brief Devuleve la cantidad de afiches a cobrar que hay
* \param list Afiche*
* \param int limite
* \param int idCliente
* \return int return -1 si el array de afiches es NULL - 0 muestra correcta
